...
- **한계 및 보완점**
  1. 학습을 위한 데이터 전처리 과정에서 메모리 부족 문제 발생.
    - 원인: 너무 많은 샘플 및 특성의 수. (USER * POLICY) = (20,000 * 88,000) = 1,760,000,000 개
      - 보완방법: High cardinality의 특성 중 중요도가 덜한 특성을 제외(ex: 시도, 시군구)  및 샘플 수 줄이는 방법.
  ...
  3. 추천진행 관련 모듈 설계 한계  .
    1) 추천 시간이 평균 23초로 매우 오래걸림.
      - 원인: 유저데이터를 입력받고, 추천을 위한 model predict 인풋값으로의 전처리 작업에서 기존 정책 8만8천개의 데이터들과의 합을 만드는 과정에서 시간이 다소 소요됨.
        - 해결방안: 유저 관심정책이 있는 경우, 관심정책을 Query로 삼아 Retrieval 모듈을 통해 우선적으로 관심정책에 해당하는 정책을 선별. (관심정책을 선택하지 않은 유저의 경우, 똑같이 해당사항없음의 정책 중에서 선별)

    2) 추천 진행 결과, 거의 추천 대상과 정책이 잘 맞지 않음.
    ...
    3) 기존 훈련 진행시 모든 중복태그를 one-hot 인코딩 하지 않는 경우, training은 진행되지만, 추천을 위한 전처리 과정에서 정확하게 feature 생성이 매우 번거로움.
