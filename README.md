...
2. **태깅 모델**</br>
  #### A. 키워드 기반 label 생성    
  - user에서 추출할 feature (13개) : 성별, 나이, 시도, 시군구, 학력, 직장, 가구원 유형, 결혼, 자녀, 자녀상세, 대상특성, 관심상황특성, 중위소득, 관심정책
  - policy에서 추출할 feature (20개) : 정책ID, 정책서비스ID, 서비스명, 소관기관, 소관기관유형, 생애주기, 신청절차, 선정기준, 지원유형, 서비스목적,  지원내용, 지원대상
...
- 태깅 모델</br>
  ...
  새롭게 만든 label들의 정확도가 최대 99% ~ 최소 40% 정도임.</br>
    ...
    2. 기업에서 제공해준 태깅 답지가 존재. 그러나 100%의 정확도를 갖지 않음. 신뢰도가 어느정도인지 알 수 없음.

  - 추출된 label 기반 KoBERT 학습 (Pytorch)
    1. 키워드 기반으로 만든 label 자체의 정확도가 낮기 때문
    2. 
